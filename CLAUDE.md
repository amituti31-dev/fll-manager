...
- Firebase Hosting serves the same `web/` build under **two site IDs** in the one `fll-manger` project: the original `fll-manger.web.app` (Firebase project IDs can't be renamed) and `fll-manager-site.web.app` (added later for a less-typo'd URL). `web/firebase.json`'s `hosting` is an array with two `target`-tagged entries (`main`→`fll-manger`, `new`→`fll-manager-site`, mapped in `web/.firebaserc`), so a plain `firebase deploy --only hosting` — including the one inside `Update-All.ps1` — updates both automatically; don't reduce it back to a single hosting block or the second site goes stale. The app's own update-check calls (`main.js`'s `checkForUpdates`, `Android/lib/main.dart`'s `_checkForUpdates`) point at `fll-manager-site.web.app/version.json`.
